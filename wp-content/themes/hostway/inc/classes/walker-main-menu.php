...
/**
 * Custom styles in MenuEdit Page
 */
function ic_nav_menu_styles() {
	echo '<style type="text/css">.menu-item-edit-active .menu-item-settings{overflow: hidden;}</style>';
}

add_action( 'admin_print_scripts-nav-menus.php', 'ic_nav_menu_styles' );
 ...
 * @return string
 */
function custom_admin_nav_menu_walker_filter( $class, $menu_id ) {
	return 'Custom_Walker_Nav_Menu_Edit';
}

add_filter( 'wp_edit_nav_menu_walker', 'custom_admin_nav_menu_walker_filter', 10, 2 );

if ( ! class_exists( 'Custom_Walker_Nav_Menu_Edit' ) ) {
				...
				<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span
					...
					<span class="item-controls">
						...
						echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item',
							$item_id, remove_query_arg( $removed_args,
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo $item_id; ?>">
						<input type="checkbox" id="edit-menu-item-target-<?php echo $item_id; ?>" value="_blank"
						       name="menu-item-target[<?php echo $item_id; ?>]"<?php checked( $item->target,
							'_blank' ); ?> />
...
/**
 * Insert custom html to edit form
 *
 * @param $item_id
 * @param $item
 * @param $depth
 * @param $args
 */
function ic_nav_menu_item_custom_fields_action( $item_id, $item, $depth, $args ) {
	?>
	<div style="clear:both;overflow: hidden;padding-right: 10px;" class="field-css-html description">
		<label for="edit-menu-item-editor-<?php echo $item_id; ?>">
			<div>Custom HTML</div>
			<textarea style="width: 100%;height: 200px"
			          name="<?php echo 'menu-item-html[' . $item_id . ']'; ?>"
			          id="<?php echo 'edit-menu-item-html-' . $item_id; ?>"
			><?php echo esc_html( $item->html ); ?></textarea>
		</label>
	</div>
	<?php
}

add_action( 'wp_nav_menu_item_custom_fields', 'ic_nav_menu_item_custom_fields_action', 10, 4 );
 ...
 */
function ic_wp_update_nav_menu_item_action( $menu_id, $menu_item_db_id ) {
	if ( isset( $_POST['menu-item-html'][ $menu_item_db_id ] ) ) {
		update_post_meta( $menu_item_db_id, '_nav_menu_html', $_POST['menu-item-html'][ $menu_item_db_id ] );
	}
}

add_action( 'wp_update_nav_menu_item', 'ic_wp_update_nav_menu_item_action', 10, 2 );
 ...
 * @return object
 */
function ic_setup_nav_item_filter( $menu_item ) {

	$menu_item->html = get_post_meta( $menu_item->ID, '_nav_menu_html', true );

	return $menu_item;
}

add_filter( 'wp_setup_nav_menu_item', 'ic_setup_nav_item_filter' );
 ...
 * @return string
 */
function ic_walker_nav_menu_start_el( $item_output, $item, $depth, $args ) {
	if ( isset( $item->html ) && ! empty( $item->html ) ) {
		...
		$item_output .= $args->link_before . $item->html . $args->link_after;
		$item_output .= '</a>';
...
add_filter( 'walker_nav_menu_start_el', 'ic_walker_nav_menu_start_el', 10, 4 );
 ...
 * @return array
 */
function ic_nav_menu_css_class( $classes, $item, $args, $depth ) {

	if ( isset( $item->html ) && ! empty( $item->html ) ) {
		$classes[] = 'menu-html-content';
	}

	return $classes;
}

add_filter( 'nav_menu_css_class', 'ic_nav_menu_css_class', 10, 4 );